...
public class HolySwordLevel4RightclickedProcedure {
			...
			for (int index5 = 0; index5 < (int) (entity.getPersistentData().getDouble("range") * precision); index5++) {
				...
				IsekaiRebornMod.queueServerWork((int) delay, () -> {
								...
								(entity.getPersistentData().getDouble("width")), 0);
					{
						...
						for (Entity entityiterator : _entfound) {
							if (!(entity == entityiterator || entityiterator instanceof ExperienceOrb || entityiterator instanceof ItemEntity)) {
								entityiterator.hurt(new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(DamageTypes.GENERIC)), 100);
								if (world instanceof Level _level && !_level.isClientSide())
